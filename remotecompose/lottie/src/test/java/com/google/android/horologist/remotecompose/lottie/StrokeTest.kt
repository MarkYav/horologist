...
import com.google.android.horologist.remotecompose.lottie.format.graphicelement.styles.StrokeDashType
...
class StrokeTest {
  ...
  @Test
  fun deserializeStroke_withDashesAndLineProperties() {
    val json =
      """
      {
        "ty": "st",
    ...
    assertThat(element).isInstanceOf(Stroke::class.java)

    val stroke = element as Stroke
    ...
    assertThat(stroke.dashes).isNotNull()
    assertThat(stroke.dashes).hasSize(3)
  }
  ...
  fun createDashPathEffect_withDashesGapsAndOffset_returnsNonNullPathEffect() {
    val dashes =
      listOf(
        StrokeDash(
          type = StrokeDashType.Dash,
          name = "dash",
          length = StaticScalarProperty(value = 12f.rf),
        ),
        StrokeDash(
          type = StrokeDashType.Gap,
          name = "gap",
          length = StaticScalarProperty(value = 6f.rf),
        ),
        StrokeDash(
          type = StrokeDashType.Offset,
          name = "offset",
          length = StaticScalarProperty(value = 3f.rf),
        ),
      )
    ...
    val dashes =
      listOf(
        StrokeDash(
          type = StrokeDashType.Dash,
          name = "dash",
          length = StaticScalarProperty(value = 15f.rf),
        )
      )
  ...
  fun createDashPathEffect_withNullOrEmptyOrAllZeros_returnsNull() {
    ...
    val allZeros =
      listOf(
        StrokeDash(type = StrokeDashType.Dash, length = StaticScalarProperty(value = 0f.rf)),
        StrokeDash(type = StrokeDashType.Gap, length = StaticScalarProperty(value = 0f.rf)),
      )
  ...
  fun createDashPathEffect_withAnimatedDash_evaluatesAtCurrentFrame() {
    val keyframes =
      listOf(
        ...
        ScalarPropertyKeyframe(frame = 30f.rf, value = 20f.rf),
      )
    val animatedDash =
      listOf(
        StrokeDash(
          type = StrokeDashType.Dash,
          length = AnimatedScalarProperty(keyframes = keyframes),
        ),
        StrokeDash(type = StrokeDashType.Gap, length = StaticScalarProperty(value = 5f.rf)),
      )
