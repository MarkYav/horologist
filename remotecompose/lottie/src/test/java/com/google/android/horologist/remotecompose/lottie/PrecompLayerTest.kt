...
class PrecompLayerTest {
  ...
  private fun testTransform(name: String) =
    Transform(
      name = name,
  ...
  private fun testNullLayer(index: Int?, parent: Int?, transform: Transform) =
    NullLayer(
      index = index,
      parent = parent,
      startFrame = 0f.rf,
  ...
  fun buildAncestorTransforms_withBaseStack_prependsBaseStackToAllLayers() {
    val baseT = testTransform("base_transform")
    val t1 = testTransform("t1")
    val t2 = testTransform("t2")

    val root = testNullLayer(index = 1, parent = null, transform = t1)
    val child = testNullLayer(index = 2, parent = 1, transform = t2)
  ...
  fun precompLayer_withNonPrecompAsset_isIgnored() {
    ...
    val precompLayer = PrecompLayer(refId = "img_0", startFrame = 0f.rf, endFrame = 100f.rf)
  ...
  fun precompLayer_withMissingAsset_isHandledSafely() {
    ...
    val precompLayer = PrecompLayer(refId = "missing_ref", startFrame = 0f.rf, endFrame = 100f.rf)
    val asset = settings.assets[precompLayer.refId] as? PrecompAsset
    assertThat(asset).isNull()
  }
  ...
  fun precompLayer_recursionGuard_detectsActivePrecomps() {
    ...
    val precompLayer =
      PrecompLayer(refId = "comp_recursive", startFrame = 0f.rf, endFrame = 100f.rf)
  ...
  fun precompLayer_trackMatteResolution_insidePrecompAsset() {
    val matteLayer =
      ShapeLayer(index = 1, startFrame = 0f.rf, endFrame = 100f.rf, shapes = emptyList())
    val maskedLayer =
      ShapeLayer(
        index = 2,
        startFrame = 0f.rf,
