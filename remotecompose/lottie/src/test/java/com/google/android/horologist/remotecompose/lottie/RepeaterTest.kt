...
class RepeaterTest {
  ...
  fun repeater_jsonDeserialization() {
    ...
    assertThat((repeater.copies as StaticScalarProperty).value).isEqualTo(5f)
    assertThat((repeater.offset as StaticScalarProperty).value).isEqualTo(1f)
    assertThat(repeater.composite).isEqualTo(CompositeMode.Below)
    assertThat(repeater.transform).isNotNull()
    assertThat((repeater.transform?.startOpacity as StaticScalarProperty).value).isEqualTo(100f)
    assertThat((repeater.transform?.endOpacity as StaticScalarProperty).value).isEqualTo(0f)
  }
  ...
  fun repeaterExponentialScale_withFractionalOffset_avoidsNaN() {
    val subpath =
      RemoteBezierValue(
        closed = true,
        vertices = listOf(listOf(10f.rf, 0f.rf)),
        inTangents = listOf(listOf(0f.rf, 0f.rf)),
        outTangents = listOf(listOf(0f.rf, 0f.rf)),
      )
    val lottiePath = RemoteLottiePath(listOf(subpath))
    ...
    val repeater =
      Repeater(
        copies = StaticScalarProperty(value = 2f),
        ...
        transform =
          Transform(
            anchorPoint = StaticPositionProperty(value = listOf(0f, 0f)),
            positionTranslation = StaticPositionProperty(value = listOf(0f, 0f)),
            scale = StaticVectorProperty(value = listOf(-100f, 100f)), // Negative scale
            rotation = StaticScalarProperty(value = 0f),
          ),
      )
    ...
    val path0 = results[0].shape as RemoteLottiePath
    val x0 = path0.path[0].vertices[0][0].constantValueOrNull
    assertThat(x0).isNotNull()
