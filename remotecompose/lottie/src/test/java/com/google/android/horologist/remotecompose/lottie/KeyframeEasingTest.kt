...
class KeyframeEasingTest {
  ...
  @Test
  fun subframeLutEasing_interpolatesFractionalProgress_accurateWithinTolerance() {
    // Test cubic bezier easing with asymmetric tangents
    val a = 0.25f
    val b = 0.1f
    val c = 0.25f
    val d = 1.0f
    ...
    val fractionalFrames = listOf(0.5f, 2.3f, 5.75f, 10.5f, 15.2f, 19.8f)
    for (f in fractionalFrames) {
      val remoteVal = lookupValueInBezier(a, b, c, d, duration, f.rf)
      val expected = easing.transform(f / duration)
      assertThat(remoteVal.constantValue).isWithin(0.005f).of(expected)
    }

    // Zero or negative duration boundary
    val zeroDuration = lookupValueInBezier(a, b, c, d, 0f, 5.rf)
    assertThat(zeroDuration.constantValue).isEqualTo(0f)
  }

  @Test
  fun scalarHoldKeyframe_maintainsInitialValueUntilNextKeyframe() {
    val scalarWithHold =
      AnimatedScalarProperty(
        keyframes =
          listOf(
            ScalarPropertyKeyframe(frame = 0f, value = 10f, hold = true),
            ScalarPropertyKeyframe(frame = 10f, value = 50f),
          )
      )

    val atFrame5 = animateScalar(scalarWithHold, LottieSettings(5.rf, emptySlotMap))
    val atFrame10 = animateScalar(scalarWithHold, LottieSettings(10.rf, emptySlotMap))

    assertThat(atFrame5.constantValue).isEqualTo(10f)
    assertThat(atFrame10.constantValue).isEqualTo(50f)
  }

  @Test
  fun vectorHoldKeyframe_maintainsInitialVectorUntilNextKeyframe() {
    val vectorWithHold =
      AnimatedVectorProperty(
        keyframes =
          listOf(
            VectorPropertyKeyframe(frame = 0f, value = listOf(10f, 20f), hold = true),
            VectorPropertyKeyframe(frame = 10f, value = listOf(100f, 200f)),
          )
      )

    val atFrame5 = animateVector(vectorWithHold, LottieSettings(5.rf, emptySlotMap))
    val atFrame10 = animateVector(vectorWithHold, LottieSettings(10.rf, emptySlotMap))

    assertThat(atFrame5.map { it.constantValue }).isEqualTo(listOf(10f, 20f))
    assertThat(atFrame10.map { it.constantValue }).isEqualTo(listOf(100f, 200f))
  }
