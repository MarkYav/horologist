...
import com.google.android.horologist.remotecompose.lottie.renderer.modifiers.CubicSegment
import com.google.android.horologist.remotecompose.lottie.renderer.modifiers.Vec2
import com.google.android.horologist.remotecompose.lottie.renderer.modifiers.evaluateTrimmedBezier
import com.google.android.horologist.remotecompose.lottie.renderer.modifiers.toBezierValue
import com.google.android.horologist.remotecompose.lottie.renderer.modifiers.toCubicSegments
import com.google.android.horologist.remotecompose.lottie.renderer.modifiers.trimBezierValue
...
class TrimPathEvaluatorTest {
  ...
  fun pointOperations() {
    ...
    val dist = Vec2(0f, 0f).distanceTo(Vec2(3f, 4f))
  ...
  fun linearSegmentLengthAndEvaluation() {
    val seg =
      CubicSegment(p0 = Vec2(0f, 0f), p1 = Vec2(0f, 0f), p2 = Vec2(100f, 0f), p3 = Vec2(100f, 0f))
  ...
  fun bezierValueConversionRoundtrip() {
    val original =
      testBezier(
        closed = false,
        vertices = listOf(pt(0f, 0f), pt(100f, 100f)),
    ...
    assertThat(converted.vertices[0].xVal).isEqualTo(0f)
    assertThat(converted.vertices[1].xVal).isEqualTo(100f)
  }
  ...
  fun trimBezierValueLinearLine() {
    ...
    assertThat(fullTrim[0].vertices[0].xVal).isWithin(0.01f).of(0f)
    assertThat(fullTrim[0].vertices[1].xVal).isWithin(0.01f).of(100f)
    ...
    assertThat(invFullTrim[0].vertices[0].xVal).isWithin(0.01f).of(0f)
    assertThat(invFullTrim[0].vertices[1].xVal).isWithin(0.01f).of(100f)
    ...
    assertThat(halfTrim[0].vertices[0].xVal).isWithin(0.01f).of(0f)
    assertThat(halfTrim[0].vertices[1].xVal).isWithin(0.01f).of(50f)
    ...
    assertThat(degenTrim[0].vertices[0].xVal).isWithin(0.01f).of(0f)
    assertThat(degenTrim[0].vertices[1].xVal).isWithin(0.01f).of(0f)
  }
  ...
  fun trimBezierValue_multiSegment_preservesTopologyAcrossDifferentTrimRanges() {
    ...
    val circle =
      testBezier(
        closed = true,
        vertices = listOf(pt(50f, 0f), pt(100f, 50f), pt(50f, 100f), pt(0f, 50f)),
        inTangents = listOf(pt(-cp, 0f), pt(0f, -cp), pt(cp, 0f), pt(0f, cp)),
        outTangents = listOf(pt(cp, 0f), pt(0f, cp), pt(-cp, 0f), pt(0f, -cp)),
      )
  ...
  fun trimBezierValue_multiSegment_evaluatedPointsLieOnArc() {
    ...
    val distFromCenter = Vec2(50f, 50f).distanceTo(midPt)
  ...
  fun trimBezierValue_whenStartEqualsEnd_preservesDegenerateStructureWithoutThrowing() {
    ...
    assertThat(collapsedTrim[0].vertices[0].xVal).isWithin(0.01f).of(50f)
    assertThat(collapsedTrim[0].vertices[1].xVal).isWithin(0.01f).of(50f)
  }
  ...
  fun evaluateTrimmedBezier_staticTrimPath_producesTrimmedPath() {
    ...
    val trim =
      TrimPath(
        start = StaticScalarProperty(value = 20f.rf),
        end = StaticScalarProperty(value = 80f.rf),
    ...
    assertThat(sub.vertices.first().xVal).isWithin(0.01f).of(20f)
    assertThat(sub.vertices.last().xVal).isWithin(0.01f).of(80f)
  }
  ...
  fun evaluateTrimmedBezier_animatedTrimPath_evaluatesAtKeyframe() {
    ...
    val keyframes =
      listOf(
        ...
        ScalarPropertyKeyframe(frame = 10f.rf, value = 100f.rf),
      )
    val trim =
      TrimPath(
    ...
    assertThat(trimmed10.first().vertices.last().xVal).isWithin(0.01f).of(100f)
  }
  ...
  fun trimPath_jsonDeserialization() {
    ...
    assertThat((trim.start as StaticScalarProperty).value.constantValueOrNull).isEqualTo(10f)
    assertThat((trim.end as StaticScalarProperty).value.constantValueOrNull).isEqualTo(90f)
    assertThat((trim.offset as StaticScalarProperty).value.constantValueOrNull).isEqualTo(180f)
  }
  ...
  fun gatherShapes_withTrimPath_appliesTrimToSiblingPaths() {
    ...
    val trim =
      TrimPath(
        start = StaticScalarProperty(value = 10f.rf),
        end = StaticScalarProperty(value = 90f.rf),
      )
    val stroke =
      Stroke(
        color = StaticColorProperty(value = Color.White.rc),
        strokeWidth = StaticScalarProperty(value = 2f.rf),
    ...
    val shape = styledShapes[0].shapes[0]
    val sub =
      when (shape) {
        is RemoteCompiledGeometry -> shape.bezierSubpaths?.first()!!
        is RemoteDynamicGeometry -> shape.path.first()
        else -> error("Unexpected shape type: ${shape::class.java}")
      }
    assertThat(sub.vertices.first().xVal).isWithin(0.01f).of(10f)
    assertThat(sub.vertices.last().xVal).isWithin(0.01f).of(90f)
  }
}
