...
internal class RemoteStroke(
  ...
  constructor(
    strokeColor: RemoteColor,
    strokeWidth: RemoteFloat,
    opacity: RemoteFloat = 100f.rf,
    lineCap: LineCap = LineCap.Round,
    lineJoin: LineJoin = LineJoin.Round,
    miterLimit: RemoteFloat?,
    dashPattern: PathEffect? = null,
  ) : this(
    ...
    lineCap = lineCap,
    lineJoin = lineJoin,
    miterLimit = miterLimit?.constantValueOrNull ?: 0f,
...
internal fun createDashPathEffect(
  ...
  for (dash in dashes) {
    ...
    if (dash.type == StrokeDashType.Offset) {
      phase = value
    } else {
...
private fun extractGradientColorsAndPositions(
  ...
  for (i in 0 until colorCount) {
    ...
    colors.add(RemoteColor.rgb(red = r, green = g, blue = b).copy(alpha = finalAlpha))
