...
/** Evaluates a Lottie [PolyStar] parametric shape into a [RemoteShape]. */
@SuppressLint("RestrictedApi")
internal fun polyStar(
  star: PolyStar,
  ...
  val rcPath = buildRemotePathFromBezier(listOf(subpath))
  return RemoteCompiledGeometry(rcPath, bezierSubpaths = listOf(subpath))
}
...
private fun createStarBezier(
  ...
  val vertices = ArrayList<Point>(numPoints)
  val inTangents = ArrayList<Point>(numPoints)
  val outTangents = ArrayList<Point>(numPoints)

  val zeroPoint = Point(0f.rf, 0f.rf)
  for (k in 0 until numPoints) {
    inTangents.add(zeroPoint)
    outTangents.add(zeroPoint)
  }

  vertices.add(Point((x + positionX).rf, (y + positionY).rf))
  ...
  for (i in 0 until numPoints) {
    ...
    if (innerRoundedness != 0f || outerRoundedness != 0f) {
      ...
      outTangents[i] = Point((-cp1x).rf, (-cp1y).rf)
      inTangents[targetIndex] = Point(cp2x.rf, cp2y.rf)
    }
