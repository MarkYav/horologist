...
internal fun roundBezierValue(subpath: BezierValue, radius: Float): BezierValue {
  ...
  val isClosed = subpath.closed.constantValue
  val newVertices = mutableListOf<Point>()
  val newInTangents = mutableListOf<Point>()
  val newOutTangents = mutableListOf<Point>()

  for (i in 0 until count) {
    val currPoint = Vec2.fromPoint(subpath.vertices[i])
    val currX = currPoint.x
    val currY = currPoint.y

    val inTan = subpath.inTangents.getOrNull(i)?.let { Vec2.fromPoint(it) } ?: Vec2(0f, 0f)
    val inX = inTan.x
    val inY = inTan.y

    val outTan = subpath.outTangents.getOrNull(i)?.let { Vec2.fromPoint(it) } ?: Vec2(0f, 0f)
    val outX = outTan.x
    val outY = outTan.y
    ...
    val prevPoint: Vec2? =
      when {
        i > 0 -> Vec2.fromPoint(subpath.vertices[i - 1])
        isClosed -> Vec2.fromPoint(subpath.vertices[count - 1])
        else -> null
      }

    val nextPoint: Vec2? =
      when {
        i < count - 1 -> Vec2.fromPoint(subpath.vertices[i + 1])
        isClosed -> Vec2.fromPoint(subpath.vertices[0])
        else -> null
      }

    if (prevPoint == null || nextPoint == null || !isSharp) {
      newVertices.add(Point(currX.rf, currY.rf))
      newInTangents.add(Point(inX.rf, inY.rf))
      newOutTangents.add(Point(outX.rf, outY.rf))
    } else {
      ...
      if (lenPrev < 0.0001f || lenNext < 0.0001f) {
        ...
        newVertices.add(Point(pStartX.rf, pStartY.rf))
        newInTangents.add(Point(0f.rf, 0f.rf))
        newOutTangents.add(Point(outTanStartX.rf, outTanStartY.rf))

        // Add end vertex of rounded corner
        newVertices.add(Point(pEndX.rf, pEndY.rf))
        newInTangents.add(Point(inTanEndX.rf, inTanEndY.rf))
        newOutTangents.add(Point(0f.rf, 0f.rf))
      }
    }
  }
...
internal fun evaluatePathGeometry(
  ...
  for (f in sampleTimes) {
    ...
    if (hasTrim && isTrimAnimated) {
      ...
      val inTangent =
        primaryScalarKf?.inTangent?.let { KeyframeEasing(it.x, it.y) } ?: bezierKf?.inTangent
      val outTangent =
        primaryScalarKf?.outTangent?.let { KeyframeEasing(it.x, it.y) } ?: bezierKf?.outTangent
      val hold = primaryScalarKf?.hold ?: bezierKf?.hold ?: false.rb
  ...
  return animateBezierSubpaths(animatedEvaluatedBezier, animationSettings)
}
