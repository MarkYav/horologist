...
/** Calculates local layer time for layer properties. */
...
internal fun Layer(
  ...
  if (layer.hidden.constantValueOrNull == true) {
    return
  }

  val startFrame = layer.startFrame.constantValueOrNull ?: 0f
  val endFrame = layer.endFrame.constantValueOrNull ?: Float.MAX_VALUE
    ...
    LayerType.Precomposition -> {
      ...
      val localFrame =
        if (precompLayer.timeRemap != null) {
          ...
          calculateLocalFrame(currentFrame, precompLayer.startTime, precompLayer.timeStretch)
        }
      val precompOpacity =
