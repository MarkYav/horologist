    ...
    when (shape) {
      ...
      is RemoteCompiledGeometry -> {
        if (shape.bezierSubpaths != null) {
          val newSubpaths =
            shape.bezierSubpaths.map { subpath -> applyPuckerBloatToSubpath(subpath, amount) }
          RemoteDynamicGeometry(newSubpaths, shape.fillRule)
        } else {
          shape
        }
...
private fun applyPuckerBloatToSubpath(
  ...
  for (v in subpath.vertices) {
    sumX += v.x.constantValueOrNull ?: 0f
    sumY += v.y.constantValueOrNull ?: 0f
  }
  ...
  for (i in 0 until count) {
    ...
    newVertices.add(Point(newVx.rf, newVy.rf))
    newInTangents.add(Point(newInX.rf, newInY.rf))
    newOutTangents.add(Point(newOutX.rf, newOutY.rf))
  }
