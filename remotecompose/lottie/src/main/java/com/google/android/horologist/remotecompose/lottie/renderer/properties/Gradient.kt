...
internal fun GradientValue.toRemote(): RemoteGradientValue {
  val colorCount = colorStops.size
  val flatList = buildList {
    for (stop in colorStops) {
      add(stop.offset.rf)
      add(stop.color.red)
      add(stop.color.green)
      add(stop.color.blue)
    }
    for (stop in transparencyStops) {
      add(stop.offset.rf)
      add(stop.alpha)
    }
  }
  return RemoteGradientValue(numberOfColors = colorCount, values = flatList)
}
  ...
  return when (gradient) {
    ...
    is AnimatedGradientProperty -> {
      ...
      if (gradient.keyframes.size == 1) {
        val single =
          gradient.keyframes[0].startValue.firstOrNull() ?: GradientValue(emptyList(), emptyList())
        return single.toRemote()
      }

      val firstKeyframe = gradient.keyframes[0]
      val firstVal =
        firstKeyframe.startValue.firstOrNull() ?: GradientValue(emptyList(), emptyList())
      val firstRemote = firstVal.toRemote()
      val numberOfColors = gradient.numberOfColors ?: firstRemote.numberOfColors

      val animationSegments = mutableListOf<GradientAnimationSegment>()
      if (firstKeyframe.time != 0f) {
        animationSegments.add(GradientAnimationSegment(startFrame = 0f, value = firstRemote.values))
      }

      for (i in 0 until gradient.keyframes.size - 1) {
        val startKeyframe = gradient.keyframes[i]
        val endKeyframe = gradient.keyframes[i + 1]
        val duration = endKeyframe.time - startKeyframe.time
        val frameInAnimation = animationSettings.currentFrame - startKeyframe.time.rf

        val startGradient = (startKeyframe.startValue.firstOrNull() ?: firstVal).toRemote()
        val endGradient =
          (endKeyframe.startValue.firstOrNull()
              ?: startKeyframe.startValue.firstOrNull()
              ?: firstVal)
            .toRemote()

        val segmentValues =
          if (startKeyframe.hold?.constantValue == true || duration <= 0f) {
            ...
            startGradient.values.mapIndexed { index, startCoord ->
              val endCoord = endGradient.values.getOrElse(index) { startCoord }
              lerp(startCoord, endCoord, progress)
            }
          }

        animationSegments.add(GradientAnimationSegment(startKeyframe.time, segmentValues))
      }
