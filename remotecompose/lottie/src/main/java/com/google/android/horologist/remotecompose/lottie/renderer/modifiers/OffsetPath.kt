    ...
    when (shape) {
      ...
      is RemoteCompiledGeometry -> {
        if (shape.bezierSubpaths != null) {
          val newSubpaths =
            shape.bezierSubpaths.map { subpath ->
              applyOffsetToSubpath(subpath, amount, offsetPath.lineJoin, miterLimit)
            }
          RemoteDynamicGeometry(newSubpaths, shape.fillRule)
        } else {
...
private fun applyOffsetToSubpath(
  ...
  for (i in 0 until count) {
    ...
    val x0 = subpath.vertices[i].x.constantValueOrNull ?: 0f
    val y0 = subpath.vertices[i].y.constantValueOrNull ?: 0f
    val x1 = subpath.vertices[nextIdx].x.constantValueOrNull ?: 0f
    val y1 = subpath.vertices[nextIdx].y.constantValueOrNull ?: 0f
    signedArea += (x0 * y1 - x1 * y0)
  }
  val isClockwise = signedArea > 0f

  val edgeNormals = mutableListOf<Vec2>()
  val edgeDirs = mutableListOf<Vec2>()
  ...
  for (i in 0 until segmentCount) {
    ...
    if (len > 0.0001f) {
      val ux = dx / len
      val uy = dy / len
      edgeDirs.add(Vec2(ux, uy))
      val nx = if (isClockwise) uy else -uy
      val ny = if (isClockwise) -ux else ux
      edgeNormals.add(Vec2(nx, ny))
    } else {
      edgeDirs.add(Vec2(1f, 0f))
      edgeNormals.add(Vec2(0f, 1f))
    }
  }
  ...
  if (subpath.closed) {
    for (i in 0 until count) {
      ...
      val pPrev = Vec2(vx + nPrev.x * amount, vy + nPrev.y * amount)
      val pCurr = Vec2(vx + nCurr.x * amount, vy + nCurr.y * amount)
      ...
      val offsetVertex =
        if (abs(det) > 0.001f) {
          ...
          if (miterDist > miterLimit * abs(amount)) {
            pCurr
          } else {
            Vec2(ix, iy)
          }
      ...
      newVertices.add(offsetVertex.toPoint())
      newInTangents.add(subpath.inTangents.getOrNull(i) ?: Point(0f.rf, 0f.rf))
      newOutTangents.add(subpath.outTangents.getOrNull(i) ?: Point(0f.rf, 0f.rf))
    }
  } else {
    for (i in 0 until count) {
      val vx = subpath.vertices[i].x.constantValueOrNull ?: 0f
      ...
      val offsetVertex =
        if (i == 0 && edgeNormals.isNotEmpty()) {
          val n = edgeNormals[0]
          Vec2(vx + n.x * amount, vy + n.y * amount)
          ...
          val nCurr = edgeNormals[i]
          Vec2(vx + (nPrev.x + nCurr.x) * 0.5f * amount, vy + (nPrev.y + nCurr.y) * 0.5f * amount)
        } else {
          Vec2(vx, vy)
        }
