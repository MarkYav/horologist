...
private fun applyZigZagToSubpath(
  ...
  for (i in 0 until segmentCount) {
    ...
    val p0x = subpath.vertices[i].x.constantValueOrNull ?: 0f
    val p0y = subpath.vertices[i].y.constantValueOrNull ?: 0f
    val p1x = subpath.vertices[nextIdx].x.constantValueOrNull ?: 0f
    val p1y = subpath.vertices[nextIdx].y.constantValueOrNull ?: 0f

    val dx = p1x - p0x
    val dy = p1y - p0y
    val len = hypot(dx, dy)

    val p0InTan = subpath.inTangents.getOrNull(i) ?: Point(0f.rf, 0f.rf)
    val p0OutTan = subpath.outTangents.getOrNull(i) ?: Point(0f.rf, 0f.rf)

    newVertices.add(Point(p0x.rf, p0y.rf))
    ...
    if (len > 0.0001f) {
      ...
      for (j in 1..intermediateCount) {
        ...
        newVertices.add(Point(vx.rf, vy.rf))

        if (pointType == ZigZagType.Smooth) {
          val outX = ux * tanLen
          val outY = uy * tanLen
          newInTangents.add(Point((-outX).rf, (-outY).rf))
  ...
  if (!subpath.closed) {
    ...
    val lastIn = subpath.inTangents.getOrNull(lastIdx) ?: Point(0f.rf, 0f.rf)
    val lastOut = subpath.outTangents.getOrNull(lastIdx) ?: Point(0f.rf, 0f.rf)
