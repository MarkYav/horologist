...
internal fun transformRepeaterBezierValue(
  ...
  val scaleXK =
    scaleX.constantValueOrNull?.let { sx ->
      if (sx >= 0f) {
        Math.pow(sx.toDouble(), k.toDouble()).toFloat().rf
      } else {
        val sign = if (k.toInt() % 2 != 0) -1f else 1f
        (sign * Math.pow(abs(sx.toDouble()), k.toDouble()).toFloat()).rf
      }
    } ?: if (k == 0f) 1f.rf else if (k == 1f) scaleX else scaleX

  val scaleYK =
    scaleY.constantValueOrNull?.let { sy ->
      if (sy >= 0f) {
        Math.pow(sy.toDouble(), k.toDouble()).toFloat().rf
      } else {
        val sign = if (k.toInt() % 2 != 0) -1f else 1f
        (sign * Math.pow(abs(sy.toDouble()), k.toDouble()).toFloat()).rf
      }
